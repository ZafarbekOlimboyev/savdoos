          ...
          <div className="card" style={{ padding: 18 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={onSearchKey} placeholder="Chek shtrix kodi yoki ID..." autoFocus
                style={{ width: "100%", height: 52, padding: "0 14px 0 48px", border: "1.5px solid #d9d5f2", borderRadius: 12, background: "#faf9ff", fontSize: 14, fontWeight: 500, outline: "none", boxSizing: "border-box" }} />
            </div>
            <div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 8 }}>Skanerlang, Enter bosing yoki pastdan tanlang</div>
          </div>
          <div style={{ fontSize: 12, color: "#a2a7b8", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", margin: "20px 4px 10px" }}>So'nggi sotuvlar</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {(recent.data || []).map((r) => {
              const on = found?.id === r.id;
              return (
                <button key={r.id} onClick={() => select(r.receipt_no)} style={{ textAlign: "left", cursor: "pointer", display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderRadius: 12, border: `1.5px solid ${on ? "var(--accent)" : "#eef0f5"}`, background: on ? "#f6f5fd" : "#fff" }}>
                  <div style={{ width: 38, height: 38, borderRadius: 10, background: "#f4f5fa", color: "#8b91a4", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>🧾</div>
        ...
        <div className="scroll" style={{ flex: 1, minWidth: 0, overflowY: "auto", padding: "24px 28px" }}>
          {done ? (
            <div style={{ maxWidth: 460, margin: "40px auto 0", textAlign: "center" }}>
              <div style={{ width: 82, height: 82, margin: "0 auto 18px", borderRadius: "50%", background: "#e9f7ef", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 44 }}>✅</div>
              ...
              <div className="card" style={{ marginTop: 20, textAlign: "left", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px" }}>
                ...
                <span style={{ fontSize: 22, fontWeight: 800, color: "#c93a3e" }} className="tabular">{fmt(done.amount)}</span>
              </div>
              <button className="btn btn-primary" style={{ width: "100%", height: 52, marginTop: 20 }} onClick={reset}>↺ Yangi qaytarish</button>
            </div>
          ) : !found ? (
            <div style={{ height: "100%", minHeight: 420, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", color: "#b3b8c9" }}>
              <div style={{ fontSize: 40 }}>▮▮▮</div>
              <div style={{ fontSize: 15, fontWeight: 600, color: "#8b91a4", marginTop: 12 }}>Chekni skaner qiling</div>
            ...
            <div style={{ maxWidth: 620 }}>
              <div className="card" style={{ marginBottom: 16 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#5b6072" }}>JAMI {fmt(found.total)}</span>
                </div>
                <div style={{ marginTop: 16, padding: 14, border: "1px dashed #e2e4ee", borderRadius: 12 }}>
                  ...
                  <div style={{ textAlign: "center", fontSize: 12, letterSpacing: 4, color: "#5b6072", marginTop: 8 }} className="tabular">{found.uid}</div>
                </div>
              </div>

              {isCredit && (
                <div style={{ display: "flex", gap: 11, padding: "14px 16px", marginBottom: 16, borderRadius: 14, background: "#fef3e2", color: "#8a5a12" }}>
                ...
                <div style={{ padding: "18px 20px 14px" }}>
                  ...
                  <div style={{ position: "relative", marginTop: 12 }}>
                    ...
                    <input value={scanVal} onChange={(e) => setScanVal(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") scanCode(scanVal); }} placeholder="Mahsulot barcode'ini skaner qiling..."
                      style={{ width: "100%", height: 44, padding: "0 14px 0 40px", border: "1.5px solid #d9d5f2", borderRadius: 11, background: "#faf9ff", fontSize: 13.5, outline: "none", boxSizing: "border-box" }} />
                  </div>
                  {scanErr && <div style={{ display: "flex", gap: 9, marginTop: 10, padding: "11px 13px", borderRadius: 11, background: "#fdecec", color: "#c93a3e", fontSize: 12.5, lineHeight: 1.45 }}>⚠️ <span>{scanErr}</span></div>}
                </div>
                {found.items.map((it, i) => {
                  const hasBc = !!it.barcode, conf = !!confirmed[i], q = qty[i] || 0;
                  const tag = !hasBc ? ["Tarozili — skanersiz", "#eef0f5", "#6b7183"] : conf ? ["Skaner tasdiqlandi", "#e9f7ef", "#12915a"] : ["Skaner talab qilinadi", "#fef3e2", "#b8730c"];
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 20px", borderTop: "1px solid #f4f5f9" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 2, background: "#f7f8fb", border: "1px solid #e6e8f0", borderRadius: 9, padding: 2 }}>
                        <button onClick={() => bump(i, -1, it.qty)} style={{ width: 28, height: 28, border: "none", background: "none", cursor: "pointer", color: "#5b6072", borderRadius: 7, fontSize: 16 }}>−</button>
                        <span style={{ minWidth: 28, textAlign: "center", fontSize: 14, fontWeight: 700, color: q > 0 ? "#c93a3e" : "#c7cbd9" }}>{q}</span>
                        <button onClick={() => (hasBc ? scanCode(it.barcode) : bump(i, 1, it.qty))} style={{ height: 28, padding: hasBc ? "0 10px" : 0, width: hasBc ? "auto" : 28, border: "none", background: "#efedfb", cursor: "pointer", color: "#5a4bc4", borderRadius: 7, fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", gap: 5 }}>{hasBc ? "▮ Skanerla" : "+"}</button>
                      </div>
                      <div style={{ width: 96, textAlign: "right", fontSize: 14, fontWeight: 700, color: q > 0 ? "#c93a3e" : "#c7cbd9" }} className="tabular">{q > 0 ? "−" + fmt(q * it.unit_price) : "0"}</div>
                    </div>
                  );
                })}
                <div style={{ padding: "16px 20px", borderTop: "1px solid #f4f5f9" }}>
                  <div style={{ fontSize: 12.5, fontWeight: 600, color: "#6b7183", marginBottom: 9 }}>Qaytarish sababi</div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
                    {REASONS.map(([k, l]) => (
                      <button key={k} onClick={() => setReason(k)} style={{ height: 38, padding: "0 14px", borderRadius: 10, cursor: "pointer", fontSize: 12.5, fontWeight: 600, border: `1px solid ${reason === k ? "var(--accent)" : "#e6e8f0"}`, background: reason === k ? "var(--accent-soft)" : "#fff", color: reason === k ? "var(--accent-ink)" : "#8b91a4" }}>{l}</button>
                    ))}
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ flex: 1 }}><div style={{ fontSize: 13, color: "#3a3f52", fontWeight: 600 }}>Mahsulot omborga qaytarilsin</div><div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 1 }}>Nuqsonli/muddati o'tgan bo'lsa o'chiring — zaxiraga qo'shilmaydi</div></div>
                    <button onClick={() => setToStock((v) => !v)} style={{ width: 48, height: 28, border: "none", borderRadius: 15, cursor: "pointer", background: toStock ? "var(--accent)" : "#d3d6e2", position: "relative" }}><span style={{ position: "absolute", top: 3, left: toStock ? 23 : 3, width: 22, height: 22, borderRadius: "50%", background: "#fff" }} /></button>
                  </div>
                </div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 20px", background: "#faf9ff", borderTop: "1px solid #f1f2f7" }}>
                  <div><div style={{ fontSize: 12.5, color: "var(--muted)" }}>{isCredit ? "Mijoz qarzidan ayiriladi" : "Qaytariladigan summa · " + (M[found.method] || found.method)}</div><div style={{ fontSize: 28, fontWeight: 800, color: "#c93a3e", marginTop: 2 }} className="tabular">−{fmt(refund)}</div></div>
                  <button onClick={confirm} disabled={busy || refund <= 0} style={{ height: 54, padding: "0 26px", border: "none", borderRadius: 13, cursor: "pointer", fontSize: 15, fontWeight: 700, color: "#fff", background: refund > 0 ? "#c93a3e" : "#d6b3b4" }}>↩ Qaytarishni tasdiqlash</button>
                </div>
              </div>
            </div>
          )}
