...
const ACT_COLOR: Record<string, [string, string]> = { create: ["#e9f7ef", "#12915a"], update: ["#eef0f5", "#5b6072"], delete: ["#fdecec", "#c93a3e"] };
    ...
    <main className="main">
      ...
      <div className="scroll" style={{ flex: 1, padding: 24 }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
          {FILTERS.map(([k, l]) => {
            const on = entity === k;
            return <button key={k} onClick={() => setEntity(k)} style={{ height: 38, padding: "0 15px", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: "pointer", border: `1px solid ${on ? "var(--accent)" : "#e6e8f0"}`, background: on ? "var(--accent)" : "#fff", color: on ? "#fff" : "#5b6072" }}>{l}</button>;
          })}
        </div>
        ...
        <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {rows.map((r, i) => {
                const c = ACT_COLOR[r.action] || ["#eee", "#555"];
                return (
                  <tr key={i}>
                    ...
                    <td style={{ ...td, color: "#5b6072" }}>{r.entity_label}</td>
                    <td style={{ ...td, fontWeight: 600 }}>{r.name || "—"}</td>
                    <td style={{ ...td, textAlign: "right", color: "#8b91a4" }}>{new Date(r.at).toLocaleString("ru-RU")}</td>
                  </tr>
                );
              })}
            </tbody>
