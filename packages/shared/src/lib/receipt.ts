...
// To'lov usuli yorlig'i — do'kon tiliga tarjima (pay.* kalitlari, chekda mahalliy tilda chiqsin).
...
export function printReceipt(input: ReceiptData): void {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    * { font-family: "Consolas", monospace; }
    ...
    <div class="row"><span>${T("receipt.receipt")}: ${escapeHtml(r.receipt_no)}</span><span>${escapeHtml(r.cashier)}</span></div>
    <div class="row"><span>${escapeHtml(r.date)}</span></div>
    <div class="line"></div>
    <table>${rows}</table>
    <div class="line"></div>
    <div class="tot"><span>${T("receipt.total")}</span><span>${fmt(r.total)}</span></div>
    <div class="row"><span>${T("receipt.payment")}: ${T("pay." + r.method)}</span></div>
    ${r.method === "cash" && r.given > 0 ? `<div class="row"><span>${T("receipt.given")}</span><span>${fmt(r.given)}</span></div><div class="row"><span>${T("receipt.change")}</span><span>${fmt(r.change)}</span></div>` : ""}
    ${r.offline ? `<div class="badge">${T("receipt.offlineSyncing")}</div>` : ""}
    <div class="foot">${escapeHtml(r.footer || T("receipt.footerDefault"))}<br/>SavdoOS</div>
  </body></html>`;
