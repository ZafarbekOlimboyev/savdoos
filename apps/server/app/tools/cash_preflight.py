...
Operator: `railway run --service savdoos python -m app.tools.cash_preflight [--company-id <uuid>]
            [--mapping <path>] [--json]`

HECH NARSA yozmaydi. Mavjud read-only asboblarни orkestrlaydi (phase0/phase1/compare_engine):
  PG readiness · inventory (companies/branches/terminals/open shifts) · FIZIK drawer model (har branch
  uchun fizik checkout/TILL soni + source: OPERATOR_MAPPING/EXISTING/TERMINAL/AMBIGUOUS) · currency
  muammolari · shadow reconcile · fizik-drawer finding (MULTI_PHYSICAL_DRAWER_UNRESOLVED) · BLOCK/REVIEW.

--mapping <path>: operator explicit TILL mapping (AMBIGUOUS branch'larni resolve qiladi).
...
def run(db, engine, company_id, *, mapping, as_json: bool) -> int:
    ...
    multi = ce.multi_cashier_till_finding(db, company_id, mapping=mapping)
    ...
    till_maps = [m for m in mappings if m.proposed_type == "TILL"]
    safe_maps = [m for m in mappings if m.proposed_type == "SAFE"]

    report = {
        "kind": "CASH_PREFLIGHT",
        "readiness": readiness,
        "inventory": inv,
        "physical_tills": [m.as_dict() for m in till_maps if m.confidence != "AMBIGUOUS"],
        "branch_safes": [m.as_dict() for m in safe_maps],
        "ambiguous_branches": [m.as_dict() for m in till_maps if m.confidence == "AMBIGUOUS"],
        "open_shift_mappings": open_rows,
        "open_shift_blocked": [r for r in open_rows if r.get("blocked")],
        "physical_drawer_finding": multi,
...
def _print_human(r: dict, readiness_ok: bool) -> None:
    ...
    C.out("")
    C.out(f"Physical drawer model: {len(r['physical_tills'])} TILL + {len(r['branch_safes'])} SAFE "
          f"(resolved) across scope")
    for m in r["physical_tills"][:40]:
        C.out(f"   TILL {m['checkout_code']:<16} branch={m['branch_code']} terminal={m['terminal_id'] or '-'} "
              f"source={m['source']}")
    mc = r["physical_drawer_finding"]
    C.out("")
    C.out(f"Physical-drawer finding: {mc['finding']}  ({mc['cutover_impact']})")
    C.out(f"   {mc['summary']}")
    if r["ambiguous_branches"]:
        C.out(f"   AMBIGUOUS branches: {len(r['ambiguous_branches'])} (fizik drawer/currency noaniq — "
              "operator --mapping/terminal kerak)")
    if r["open_shift_blocked"]:
        C.out(f"   OPEN shifts unmappable: {len(r['open_shift_blocked'])} (fizik TILL'ga resolve bo'lmadi)")
    C.out("")
...
def main(argv=None, *, session_factory=None, engine=None) -> int:
    ...
    p.add_argument("--mapping", default=None,
                   help="Operator explicit TILL mapping (JSON fayl) — AMBIGUOUS branch'larni resolve qiladi.")
    ...
    try:
        return run(db, eng, company_id, mapping=mapping, as_json=args.json)
    finally:
        db.close()
