...
| skipped_row_products | keep / deactivate_and_zero | o'tkazib yuborilgan qatorning mahsulotlari: operator tanlagan LINK/REACTIVATE maqsadi (keyin siyosat o'tkazgan), BLOCKED/SKIP qatorning GUID egalari (boshqa manba nomi bilan saqlangani ham) yoki yagona nomzodi (MANY_TO_ONE tufayli nishoni tozalangan qatorda ham) — hisobotda `row_targets`. CREATE qarori berilgan qatorning nomzodi bunga KIRMAYDI (rad etilgan — `binos_missing_from_source`). EXCLUDED qator (papka, o'chirish belgisi, xizmat) GUID egasi ham `binos_missing_from_source` ga tushadi: 1C'da u sotuv tovari emas |
